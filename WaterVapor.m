...
vidObj = VideoWriter('.\wv.mp4','MPEG-4');
vidObj.FrameRate=10;
vidObj.Quality=80;
open(vidObj);
...
while(i<=144)
    try
        [I,map]=imread(['http://www.jma.go.jp/en/gms/imgs_c/6/watervapor/1/',time2,m(i,1:2),m(i,4:5),'-00.png']);
...
while(i<=288)
    try
        [I,map]=imread(['http://www.jma.go.jp/en/gms/imgs_c/6/watervapor/1/',time1,m(i,1:2),m(i,4:5),'-00.png']);
        [Y, NEWMAP] = imresize(I, 0.78);
        f = figure('Visible', 'off');
        a = axes('Visible','off');
        imshow(Y,NEWMAP);
